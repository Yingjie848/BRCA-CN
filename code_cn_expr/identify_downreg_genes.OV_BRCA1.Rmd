...
## Setup input data and working directory
### Input data include RNAseq, copy number, cytobands selected, biallelic and control samples, gene2cytoband list 
...
datadir <- "data"
# file for band loss
file_band_loss <- paste(datadir,"20210129_biallelic_ctl_cytobands/identified_cytobands_Yingjie_2021-01-29/final_ov_loss.txt",sep="/")
# Rdata for expression and copy number
file_exprs_cn <- "data/ov_exprs_cn.rds"
# file DE genes
file_DE <- "data/OV_samples.fit.csv"
# set output prefix
prefix <- paste(workdir,"OV_samples",sep='/')
prefix
# output dir
outdir <- "results_OV_BRCA1"
dir.create(outdir)
```
...
file.HRD <- paste(datadir,"build_biallelic_sample_list/samples_biallelic_BRCA1.txt",sep="/")
...
### save data to a list by each gene 
...
```{r}
# filter out missing values, those caused by no sample in the corresponding group
counts <- subset(counts,!is.na(ratio1) & !is.na(ratio2) )
counts.wo_missing_values = counts
```

```{r}
# add some additional information to counts
...
### identify recurrent genes, including those in selected cytobands
* ratio1: 0.7 is defined as we expect high proportion of low expression samples in biallelic loss, higher is better, 0.7 is a good number to get more genes
* ratio2: 1.5 is defined meaning 50% higher than control
* pval: 0.05 is defined as signaificant, so we can identify significant recurrent genes

```{r}
# subset table to identify recurrent genes, significant recurrent genes on cytoband loss

thres_BLL = 0.7 # ratio1
thres_BLL_CNL = 1.5 # ratio2
